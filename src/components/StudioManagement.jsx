...
function StudioManagement() {
    const { studioName, updateStudioName, studioReputation, games } = useContext(GameContext);
    const [newStudioName, setNewStudioName] = useState(studioName);
    const [isExpanded, setIsExpanded] = useState(false);

    const handleNameChange = () => {
        if (newStudioName.trim() !== '') {
            updateStudioName(newStudioName);
    ...
    return (
        <div className="bg-kb-black p-4 rounded-lg shadow-md">
            <h1 className="text-xl font-bold text-kb-white mb-4">Studio Management</h1>
            <div className="flex justify-between items-center cursor-pointer" onClick={() => setIsExpanded(!isExpanded)}>
                <h2 className="text-xl font-bold text-kb-white">Studio Details</h2>
                <svg 
            ...
            <p className="text-kb-grey mt-2">Studio Name: {studioName}</p>
            <p className="text-kb-grey">Reputation: {studioReputation?.toFixed(2) || '0.00'}</p>
